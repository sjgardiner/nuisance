...
//********************************************************************
MicroBooNE_CC1MuNp_XSec_1DPp_nu::MicroBooNE_CC1MuNp_XSec_1DPp_nu(nuiskey samplekey) {
//********************************************************************

  // Sample overview ---------------------------------------------------
  std::string descrip = "MicroBooNE_CC1MuNp_XSec_1DPp_nu sample. \n" \
                        "Target: Ar \n" \
                        "Flux: BNB FHC numu \n" \
                        "Signal: CC 1 mu + Np \n";
  ...
  // Plot information
  fSettings.SetTitle("MicroBooNE_CC1MuNp_XSec_1DPp_nu");
  ...
  SetDataFromRootFile(inputFile, "DataXsec_pmom");
  ScaleData(1E-38);
  SetCovarFromRootFile(inputFile, "CovarianceMatrix_pmom");

  // Load smearing matrix ---------------------------------------------
  TFile* inputRootFile = TFile::Open(inputFile.c_str());
  assert(inputRootFile && inputRootFile->IsOpen());
  TH2D* hsmear = (TH2D*) inputRootFile->Get("SmearingMatrix_pmom");
  assert(hsmear);
...
bool MicroBooNE_CC1MuNp_XSec_1DPp_nu::isSignal(FitEvent* event) {
  return SignalDef::isCC1MuNp(event, EnuMin, EnuMax);
};


void MicroBooNE_CC1MuNp_XSec_1DPp_nu::FillEventVariables(FitEvent* event) {
  if (event->NumFSParticle(2212) == 0) return;
  fXVar = event->GetHMFSParticle(2212)->fP.Vect().Mag() / 1000;
};


void MicroBooNE_CC1MuNp_XSec_1DPp_nu::ConvertEventRates() {
  // Do standard conversion
  Measurement1D::ConvertEventRates();

std::cerr << "before: " << fMCHist->Integral() << std::endl;
...
std::cerr << "after: " << fMCHist->Integral() << std::endl;
}
