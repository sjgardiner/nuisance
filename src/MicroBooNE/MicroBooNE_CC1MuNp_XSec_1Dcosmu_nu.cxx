...
//********************************************************************
MicroBooNE_CC1MuNp_XSec_1Dcosmu_nu::MicroBooNE_CC1MuNp_XSec_1Dcosmu_nu(nuiskey samplekey) {
//********************************************************************

  // Sample overview ---------------------------------------------------
  std::string descrip = "MicroBooNE_CC1MuNp_XSec_1Dcosmu_nu sample. \n" \
                        "Target: Ar \n" \
                        "Flux: BNB FHC numu \n" \
                        "Signal: CC 1 mu + Np \n";
  ...
  // Plot information
  fSettings.SetTitle("MicroBooNE_CC1MuNp_XSec_1Dcosmu_nu");
  ...
  SetDataFromRootFile(inputFile, "DataXsec_muangle");
  ScaleData(1E-38);
  SetCovarFromRootFile(inputFile, "CovarianceMatrix_muangle");
...
bool MicroBooNE_CC1MuNp_XSec_1Dcosmu_nu::isSignal(FitEvent* event) {
  return SignalDef::isCC1MuNp(event, EnuMin, EnuMax);
};


void MicroBooNE_CC1MuNp_XSec_1Dcosmu_nu::FillEventVariables(FitEvent* event) {
  if (event->NumFSParticle(13) == 0) return;
  fXVar = event->GetHMFSParticle(13)->fP.Vect().CosTheta();
};


void MicroBooNE_CC1MuNp_XSec_1Dcosmu_nu::ConvertEventRates() {
